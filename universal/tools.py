...
def quickrun(algo, data=None, n=1000, **kwargs):
    ...
    if data is None:
        #n：时刻数，k：资产个数，利用多元几何布格分布生成资产价格序列
        data = random_portfolio(n=n, k=3, mu=0.0001)
    t = time()#记录算法消耗时间
    result = algo.run(data)
    # logging.debug("Time: {:.2f}s".format(time() - t))
    #
    # print(f"下面是{str(algo)}的结果")
    # print(result.summary())
    # path = r"C:\Users\shenjiayu\Desktop\result\\"+str(algo)+".jpg"
    # result.plot(**kwargs)
    # plt.savefig(path)
    # plt.show()
...
def random_portfolio(n, k, mu=0.0, sd=0.01, corr=None, dt=1.0, nan_pct=0.0):
    ...
    :param corr: Correlation matrix of assets. Default is identity.资产的相关度矩阵
    :param dt: Time step.
    :param nan_pct: Add given percentage of NaN values. Useful for testing
                    添加给定百分比的 NaN 值。对测试有用
    """
    # default values
...
def opt_weights(
    ...
    **kwargs,
    ...
    """Find best constant rebalanced portfolio(CRP) with regards to some metric.
