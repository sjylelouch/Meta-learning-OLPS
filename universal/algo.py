...
class Algo(object):
    ...
    def __init__(self, min_history: Optional[int] = None, frequency: int = 1, **kwargs):
        ...
        :param min_history: If not None, use initial weights for first min_window days. Use
            ...
            获取历史记录进行正确的参数估计；如果是nan，则在第0天使用初始权重
    ...
    def init_step(self, X):
        ...
        :param X: Entire stock returns history.整个股票收益历史序列
        """
        pass
    ...
    def step(self, x, last_b, history=None):
        ...
        increases performance.（这是一个抽象方法，在子类中实现）计算新的权重参数，如果省略历史信息，直接传递参数x和last_b
    ...
    def weights(self, X, min_history=None, log_progress=True):
        ...
        this method only at your own risk.利用此函数来更新权重参数"""
    ...
    def run(self, S, n_jobs=1, log_progress=True) -> AlgoResult:
        ...
        返回一个 AlgoResult 类型的对象。
        :params S: Absolute stock prices.股票绝对价格 DataFrame with stocks in columns.每一列表示一只股票
        :param show_progress: Log computation progress. Works only for algos with
            defined step method.记录计算进度。
        ...
        # get weights，根据输入的股票相对价格序列，获取该算法在每个时刻的权重参数，存在B矩阵当中
        if n_jobs == 1:
            try:
